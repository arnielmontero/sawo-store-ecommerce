...
async function clearStoreData() {
  ...
  // Review/ProductQuestion cascade from Product, CartLead cascades from
  // User (all onDelete: Cascade, see schema.prisma) so these lines aren't
  // strictly required — kept explicit anyway so this function reads as the
  // full list of customer-facing data being wiped, matching the
  // Notification precedent above (easy to forget a table here since
  // nothing enforces it).
