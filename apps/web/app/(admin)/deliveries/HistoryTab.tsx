...
const SORTABLE_COLUMNS: { field: ShipmentSortField; label: string }[] = [
  { field: "updatedAt", label: "Resolved" },
  { field: "totalCents", label: "Total" },
];

// Read-only — Delivered/Returned orders have nothing left to act on here,
// this is purely a browsable history (Deliveries otherwise only ever
// showed Pending/In-Transit, so completed shipments had nowhere to look
// back at).
export function HistoryTab() {
  ...
  async function handleExport() {
    const url = await exportShipmentsCsvUrl("history", {
      search: search || undefined,
      carrier: carrierFilter.length > 0 ? carrierFilter : undefined,
      country: countryFilter.length > 0 ? countryFilter : undefined,
      dateFrom: dateFrom || undefined,
  ...
  return (
    <div className="mt-4 rounded-xl border border-ink-100 bg-white">
      <div className="flex items-center justify-between gap-3 px-5 pt-4">
        <p className="text-sm font-medium text-ink-900">Delivered / Returned ({pagination.total})</p>
      </div>
      ...
      {error ? (
        ...
        <p className="px-5 py-8 text-center text-sm text-ink-500">
          {search || hasFilters ? "No delivery history matches your search or filters." : "No completed deliveries yet."}
        </p>
      ) : (
